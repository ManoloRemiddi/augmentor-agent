...
from browser import open_url, desktop_environment, DESKTOP_KEYS


def observe(app_pid=None):
    # DSH may run outside the graphical session. Resolve the verified native
    # app's environment before AT-SPI initializes its bus connection.
    environment = desktop_environment()
    for key in (*DESKTOP_KEYS, 'AT_SPI_BUS_ADDRESS'):
