...
import { diagnosticFrame } from './diagnostics.mjs'

export const HOST = 'com.augmentor.agent'
...
export function log(kind, data) {
  if(kind==='wire')data={direction:['ext->bridge','ext<-bridge','bridge->ext'].includes(data?.dir)?data.dir:null,frame:diagnosticFrame(data?.msg)}
  const entry = { t: Date.now(), kind, ...data }
  // Live DSH chunks have no durable history sequence. Give each delivery an
  // identity that survives panel replay without advancing the history cursor.
  if (kind === 'event' && entry.event?.seq == null) entry.liveId = crypto.randomUUID()
  if (kind === 'wire') {
...
export function loadStoredSelection() {
  return new Promise((resolve) => {
    try {
      chrome.storage.local.get(state.harness==='dsh'?MODEL_STORAGE_KEY:MODEL_STORAGE_KEY+'-'+state.harness, (items) => {
        const sel = items?.[state.harness==='dsh'?MODEL_STORAGE_KEY:MODEL_STORAGE_KEY+'-'+state.harness]
...
export function saveSelection(sel) {
  if (sel === null) return
  try {
    chrome.storage.local.set({ [state.harness==='dsh'?MODEL_STORAGE_KEY:MODEL_STORAGE_KEY+'-'+state.harness]: sel }, () => void chrome.runtime.lastError)
  } catch {
...
export function loadStoredSessionId() {
  return new Promise((resolve) => {
    try {
      chrome.storage.local.get(state.harness==='dsh'?SESSION_STORAGE_KEY:SESSION_STORAGE_KEY+'-'+state.harness, (items) => {
        const id = items?.[state.harness==='dsh'?SESSION_STORAGE_KEY:SESSION_STORAGE_KEY+'-'+state.harness]
...
export function saveSessionId(id) {
  try {
    chrome.storage.local.set({ [state.harness==='dsh'?SESSION_STORAGE_KEY:SESSION_STORAGE_KEY+'-'+state.harness]: id }, () => void chrome.runtime.lastError)
  } catch {
...
export function clearStoredSessionId() {
  try {
    chrome.storage.local.remove(state.harness==='dsh'?SESSION_STORAGE_KEY:SESSION_STORAGE_KEY+'-'+state.harness, () => void chrome.runtime.lastError)
  } catch {
    /* storage unavailable */
  }
}
