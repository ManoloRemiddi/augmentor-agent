...
export function handlePanelMessage(msg, sender, sendResponse) {
    ...
    ;(async()=>{
      if(!state.sessionReady){
        ...
        if(exists===false){
          ...
          await request('session.create',{sessionId:state.sessionId,cwd:state.chatCwd??state.homeDir,agentPreset:state.agentPreset})
        }
        if(state.selection)await request('session.selectModel',{sessionId:state.sessionId,provider:state.selection.provider,model:state.selection.model})
  ...
  if(msg?.type==='onboarding/start'){
    ensurePort();request('augmentor/onboarding',{topic:msg.topic,requestId:msg.requestId}).then(result=>sendResponse({ok:true,result})).catch(error=>sendResponse({ok:false,error:error.message}));return true
  }
