...
function setup(t){
 ...
 const surface=attachSurface({send:async(type,payload)=>{sent.push({type,...payload});if(type==='surface/state')return {ok:true,pinned:true};if(type==='prompt/improve')return reply();return {ok:true}},openSettings:s=>opened.push(s),onError:e=>errors.push(e),approval:()=>opened.push('approval'),state:()=>state})
...
test('actual sidebar structure matches floating header and keeps the composer above all footer controls',()=>{
 const d=new JSDOM(html).window.document
 assert.deepEqual([...d.querySelectorAll('#window-actions button')].map(b=>b.id),['newchat','save','sessions','pin','compact','more','hide'])
 ...
 assert.equal(d.querySelectorAll('#stats,#site').length,0)
 assert.equal(d.querySelector('#input').placeholder,'Ask Augmentor…')
})
test('More menu, keyboard dismissal and compact view retain the draft',async t=>{
 ...
 $('compact').click();assert.equal($('activity-orb').hidden,false);$('activity-orb').click();assert.equal($('input').value,'Unsent text')
})
...
test('failed improvement preserves text and pin reports actual saved state',async t=>{
 const {$,input,setReply,errors}=setup(t);await delay(0);input('Keep');setReply(async()=>({ok:false,error:'Unavailable'}));await $('improve').onclick();assert.equal($('input').value,'Keep');assert.deepEqual(errors,['Unavailable']);await $('pin').onclick();assert.equal($('pin').getAttribute('aria-pressed'),'false')
})
