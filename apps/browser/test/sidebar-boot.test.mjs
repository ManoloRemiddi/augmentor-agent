...
try{
 ...
 const input=d.querySelector('#input');input.focus();input.value='/news';input.setSelectionRange(5,5);input.dispatchEvent(new dom.window.Event('input'));
 await new Promise(r=>setTimeout(r,10));
 const enter=()=>input.dispatchEvent(new dom.window.KeyboardEvent('keydown',{key:'Enter',bubbles:true,cancelable:true}));
 enter();
 if(input.value!=='Immediate merged send'||promptCalls!==0||d.querySelector('.msg.user.pending'))throw Error('First Enter did not expand without sending');
 enter();
