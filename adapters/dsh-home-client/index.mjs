...
export function apply(ctx){
 for(const d of definitions)ctx.tools.register({...d,
  augmentorExecution:{effect:()=>['home_status','home_result','home_read'].includes(d.name)?'read':'change',outcome:(_args,value)=>({status:['unknown','incomplete','interrupted'].includes(value?.response?.status??value?.status)?'unknown':value?.status==='running'?'running':'completed'})},
  output:{schema:{type:'object'},render:(_args,value)=>JSON.stringify(value)},
