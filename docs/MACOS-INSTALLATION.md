...
The default destination is `~/Applications/Augmentor Agent Desktop.app`.
Use `--destination` to select another owned application directory, such as the
isolated preview destination. The installer verifies and stages the candidate,
...
An existing Augmentor bundle is retained beside the destination with a unique
`.before-<unique-id>.app` name. The JSON result gives its exact path.
...
setting `candidate` to that bundle. Its default destination is
`~/Applications/Augmentor Agent Browser Companion.app`. It contains the browser
...
In candidate 8 and later, open Settings and save a shortcut to enable its login
service. The dialog explains this before saving. The app transfers ownership to
the service, which keeps the shortcut registered after the app quits. An existing
...
```sh
installed="$HOME/Applications/Augmentor Agent Desktop.app"
...
Current evidence covers launchd bootstrap, stop, restart, native shortcut
restoration, and the installed candidate 8 Settings save button. Actual
logout/login and physical key delivery still require acceptance testing.
